...
def init_guess(setting, data, use_torso=False, **kwargs):
    ...
    fixed_scale = 1.0 if setting["fixed_scale"] is None else setting["fixed_scale"]
    ...
    model.reset_params(
        transl=init_t, global_orient=init_r, scale=init_s, betas=init_shape
    )

    init_pose = torch.zeros((1, 69), dtype=dtype).cuda()
    ### this is where we get the output of the model
    ...
    if len(keypoints) == 1:
        ...
        cam_joints = np.dot(
            setting["extris"][0], np.insert(joints.copy(), 3, 1, axis=1).T
        )
        cam_joints[2, :] += est_d
    ...
    if False:
        if kwargs.get("use_vposer"):
            ...
            init_pose = vposer.decode(setting["pose_embedding"], output_type="aa").view(
                1, -1
            )
        else:
            init_pose = torch.zeros((1, 69), dtype=dtype).cuda()
