...
def change(image_path, keyps):
    ...
    for v in range(len(image_path)):
        ...
        while True:
            ...
            cv2.resizeWindow(
                img_dir, int(imgs.shape[1] * ratio), int(imgs.shape[0] * ratio)
            )
            cv2.imshow(img_dir, imgs)
            cv2.setMouseCallback(img_dir, points_move)
            for number in range(len(mouse_list)):
                keypoints[v][0][points_index_list[number]][0:2] = mouse_list[
                    number
                ]  # 保存移动后的点
            key = cv2.waitKey(1)
            if key == 27:
...
def result2mesh(result, setting, use_vposer=True):
    ...
    if use_vposer:
        ...
        body_pose = (
            vposer.decode(pose_embedding, output_type="aa").view(1, -1)
            if use_vposer
            else None
        )
        if True:
        ...
        if True:
            result["body_pose"][:, 18:24] = 0.0
            result["body_pose"][:, 27:33] = 0.0
            result["body_pose"][:, 57:] = 0.0
...
def changeNew(image_path, keyps, results, setting):
    ...
    for v in range(len(image_path)):
        ...
        camIn = [
            [
                cam.focal_length_x.detach().squeeze().cpu().numpy(),
                0,
                cam.center[0][0].detach().squeeze().cpu().numpy(),
            ],
            [
                0,
                cam.focal_length_y.detach().squeeze().cpu().numpy(),
        ...
        imgs = render(
            verts,
            faces,
            cam.rotation.detach().cpu().squeeze().numpy(),
            cam.translation.detach().cpu().squeeze().numpy(),
            camIn,
            imgs,
            viz=False,
        )
        ...
        for p in joints2D[0]:
            imgs = cv2.circle(imgs, (int(p[0]), int(p[1])), 3, (0, 0, 255), 10)
...
def estimate_translation_from_intri(
    S, joints_2d, joints_conf, fx=5000.0, fy=5000.0, cx=128.0, cy=128.0
):
...
def surface_projection(vertices, faces, joint, exter, intri, image, op):
    ...
    intri_ = np.insert(intri, 3, values=0.0, axis=1)
    temp_v = np.insert(vertices, 3, values=1.0, axis=1).transpose((1, 0))
    ...
    temp_joint = np.insert(joint, 3, values=1.0, axis=1).transpose((1, 0))
...
def load_camera_para(file):
    ...
    for line in f:
        ...
        if len(words) == 3:
            ...
            campose_.append(
                [float(words[0]), float(words[1]), float(words[2]), float(words[3])]
            )
        else:
            pass
    ...
    for i in campose_:
        ...
        if index == 3:
            index = 0
            campose_t.append([0.0, 0.0, 0.0, 1.0])
...
def smpl_to_annotation(
    ...
    if pose_format == "coco17":
        if model_type == "smpl":
            ...
            return np.array(
                [24, 25, 26, 27, 28, 16, 17, 18, 19, 20, 21, 1, 2, 4, 5, 7, 8],
                dtype=np.int32,
            )
        else:
...
def WhichSelected(keypoints, x, y):
    ...
    if points_index != -1:
        ...
        print("the selected points is: ", (selected_point[0], selected_point[1]))
...
def keyboardCall(key, init_param=None):
    ...
    if key == ord("z"):
        if (
            my_global_orient[0, gloablOrientAxis]
            >= orient_min_value[0, gloablOrientAxis]
        ):
            my_global_orient[0, gloablOrientAxis] = (
                my_global_orient[0, gloablOrientAxis] - 0.05
            )
    if key == ord("c"):
        if (
            my_global_orient[0, gloablOrientAxis]
            <= orient_max_value[0, gloablOrientAxis]
        ):
            my_global_orient[0, gloablOrientAxis] = (
                my_global_orient[0, gloablOrientAxis] + 0.05
            )
    ...
    if flag_change_value == True:
        if key == ord("q"):
            if (
                my_body_pose[0, 3 * joints_index + keyAxis]
                >= pose_min_value[0, 3 * joints_index + keyAxis]
            ):
                my_body_pose[0, 3 * joints_index + keyAxis] = (
                    my_body_pose[0, 3 * joints_index + keyAxis] - 0.02
                )
        if key == ord("e"):
            if (
                my_body_pose[0, 3 * joints_index + keyAxis]
                <= pose_max_value[0, 3 * joints_index + keyAxis]
            ):
                my_body_pose[0, 3 * joints_index + keyAxis] = (
                    my_body_pose[0, 3 * joints_index + keyAxis] + 0.02
                )
...
def visualize_fitting(
    joints, verts, faces, camera, image_path, save=False, path="./temp_vis"
):
        ...
        for p in d2j[v][0]:
            if flag_change_value == True:
                ...
                if count == a_index:  # 选中点画成红色
                    ...
                    cv2.circle(img, (int(p[0]), int(p[1])), 3, (0, 0, 255), 10)  # 红色
                else:  # 未选中的点正常画
...
def visualize_results(
    ...
    for p in d2jD[0]:
        if flag_change_value == True:
            ...
            # )
            gimg[name] = cv2.circle(
                cv2.cvtColor(gimg[name], cv2.COLOR_BGR2RGB), (int(p[0]), int(p[1])), 3, (0, 0, 255), 10
            )            
        count = count + 1
...
def save_results(
    ...
    **kwargs
    ...
    if setting["adjustment"]:
        ...
        global points_to
        points_to = dict(zip(np.arange(0, distinat.shape[0]), distinat))
        if not use_vposer:
        ...
        model_output = model(
            global_orient=my_global_orient,
            transl=my_transl,
            return_verts=True,
            body_pose=my_body_pose,
            betas=my_betas,
        )  # ! 主要
        body_joints = model_output.joints
        verts = model_output.vertices
        ...
        if use_vposer:
            ...
            pose = np.hstack(
                (
                    my_global_orient.detach().cpu().numpy(),
                    my_body_pose.detach().cpu().numpy(),
                )
            )
            result["pose"] = pose
...
class Renderer:
    def __init__(self, resolution=(256, 256, 3), wireframe=False):
        ...
        self.colors = {
            "red": [0.8, 0.1, 0.1],
            "bule": [0.1, 0.1, 0.8],  # [.7, .7, .6],#
            "green": [0.1, 0.8, 0.1],
            "pink": [0.7, 0.7, 0.9],
            "neutral": [0.9, 0.9, 0.8],  # [.7, .7, .6],#
    ...
    def __call__(
        ...
        center = (
            np.max(np.array(mesh.vertices), axis=0)
            + np.min(np.array(mesh.vertices), axis=0)
        ) / 2.0
        size = (
            np.max(np.array(mesh.vertices), axis=0)
            - np.min(np.array(mesh.vertices), axis=0)
        ...
        if img is not None:
            ...
            image = (
                image * valid_mask * visible_weight
    ...
    def render_multiperson(
        self, verts, faces, rotation, trans, intri, img=None, viz=False
    ):
        # assert len(verts) < 5
        # Add mesh
        mesh_nodes = []
